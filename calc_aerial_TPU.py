...
'''
The input files to this code are tiled las files that contain only
time-sorted bathymetry points (las class code 26) that have been 
split into individual flight lines.    

The input files can be generated by 
running the accompanying .py file "pre_TPU_tile_processing.py" on
the standard tiles generated by NGS (which contain multiple classes 
and multiple flight lines).
'''
...
# parse data from las file using laspy
def build_las_data(inFile, start, end, thin_factor):
    ...
    points = inFile.points['point']

    t = points['gps_time'][start:end:thin_factor]
    X = points['X'][start:end:thin_factor]
    Y = points['Y'][start:end:thin_factor]
    Z = points['Z'][start:end:thin_factor]
...
def main(sbet_dir, las_dir):

    main_tic = datetime.now()

    '''
    the las files are processed in chunks to not tax memory
    when processing large files;  i found a chunk_size of
    1,000,000 worked nicely, but I imagine that might be 
    computer specific?
    '''
    chunk_size = 1000000

    '''
    thin_factor variable originally took every nth data point to make processing faster; 
    code has been improved to the point that I don't think we need
    to thin the data anymore, but it's in here anyway in case somebody
    finds it useful (I personally would get rid of the option.)
    '''
    thin_factor = 1
    ...
    #sbet_dir = r'I:\NGS_TPU\Data_for_TopoBathy_Lidar_TPU\sbet\sbet02'
    #las_dir = r'I:\NGS_TPU\Data_for_TopoBathy_Lidar_TPU\las\04_FL1604-TB-N-880_g_gpsa_rf_ip_wsf_r_adj_qc\SPLIT'


    #mat_contents_splitlas = sio.loadmat('las_tile_dirname.mat')
    #ll = mat_contents_splitlas['las_tile_dir']
    #aa = " ".join(str(x) for x in ll)
    #las_dir = aa

    #mat_contents_sbet = sio.loadmat('sbet_dirname.mat')
    #kk = mat_contents_sbet['sbet_dir']
    #bb = " ".join(str(x) for x in kk)
    #sbet_dir = bb
  #--- End of section edited by Firat------
    ...
    print('Defining observation equations and calculating Jacobian...'),
    J_tic = time.clock()
    r, p, h, a, b, w = symbols('r p h a b w')
    M, R, fR = get_M_and_R(r, p, h, a, b, w)
    fF, fJ1, fJ2, fJ3 = define_obs_eqs_and_J(R, M)
    J_toc = time.clock()
    print('Done ({} secs)'.format(J_toc - J_tic))

    las_begin_tic = datetime.now()
    tics = np.asarray([])
    ...
    #sz_del = sz_del[np.newaxis, :].T #This is how a 1D vector is transposed

    sx_del=np.delete(sx1,0) # Delete the first element wihich is 0
    #sx_del = sx_del[np.newaxis, :].T #This is how a 1D vector is transposed


    sy_del=np.delete(sy1,0) # Delete the first element wihich is 0
    #sy_del = sy_del[np.newaxis, :].T #This is how a 1D vector is transposed

    sx1=sx_del[np.newaxis, :].T
    sy1=sy_del[np.newaxis, :].T
    sz1=sz_del[np.newaxis, :].T
    arr_del = np.concatenate((arr_del, sx1,sy1,sz1), axis=1)
    #sio.savemat('coord.mat', dict(LE_post=arr_del))
    #sio.savemat('sz.mat',dict(sigz=sz_del))
