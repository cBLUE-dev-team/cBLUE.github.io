...
<<<<<<< HEAD
    for i, las in enumerate(las_files):
        ...
        num_file_points = inFile.__len__()  # len(point_records)
        num_chunks = (num_file_points / chunk_size) + 1

        print('({} of {}) {} ({} points)'.format(i+1, num_las, las.split('\\')[-1], num_file_points))

        if num_file_points != 0:

            for chunk in range(num_chunks):
                total_tic = time.clock()
                start = chunk * chunk_size
                end = min(start + chunk_size, num_file_points)

                #  get data from .las file
                build_las_tic = time.clock()
                t, x, y, z = build_las_data(inFile, start, end, thin_factor)
                build_las_toc = time.clock()
                build_las_t = build_las_toc - build_las_tic

                #  merge sbet and las data
                merge_tic = time.clock()
                min_las_t = np.min(t)
                max_las_t = np.max(t)
                ...
                if not D:  # i.e., if D is empty (las and sbet not merged)
                    ...
                    flight_lines.extend([str(i)] * sz.shape[0])
