...
class Tpu:
    ...
    #         data = in_las.reader.get_dimension(field.name)
    #         out_las.writer.set_dimension(field.name, data)
    #
    ...
    def run_tpu_multiprocessing(self, las_files, sbets):
        p = pp.ProcessPool()
        p.imap(self.calc_tpu, las_files, sbets)
        p.close()
        p.join()
